...
public class AudioEventsData : MonoBehaviour {

	//[SerializeField]
	//public AudioSource audio;	//RCC audio.time causes error
	...
	[SerializeField]
	public List<AudioPosTriggerData> triggers = new List<AudioPosTriggerData>();
	...
	public float GetCurrentValue(int curve) {
		//return GetValueAtTime(curve, audio.time);
		return GetValueAtTime(curve, GetComponent<AudioSource>().time);	//RCC fixed unity5.5 error
	}
	public float GetCurrentValue(string curve) {
		int curveIdx = (int)nameMapping[curve];
		//return GetValueAtTime(curveIdx, audio.time);
		return GetValueAtTime(curveIdx, GetComponent<AudioSource>().time);	//RCC fixed unity5.5 error
	}
	...
	public void TimeChangedManually () {
		//currentEvent = BinarySearchFirstTrigger(audio.time);
		currentEvent = BinarySearchFirstTrigger(GetComponent<AudioSource>().time);	//RCC fixed unity5.5 error
	}
	...
	void FixedUpdate () {
		...
		if(prevEvent != currentEvent && currentEvent>=0){
			SendMessage(triggers[currentEvent].methodName, triggers[currentEvent].data, SendMessageOptions.DontRequireReceiver);
		}
	}
}
