...
[CustomEditor(typeof(AudioEventsData))]
public class AudioEventsDataGUI : Editor {
	...
	public override void OnInspectorGUI () {
		bool dirty = false;
		AudioEventsData target = this.target as AudioEventsData;
		...
		if(eventsFoldout){
			for(int i=0; i<target.triggers.Count; ++i){
				...
				target.triggers[i].data = EditorGUILayout.TextField(target.triggers[i].data);
				EditorGUILayout.EndHorizontal();
			}
			EditorGUILayout.BeginHorizontal();
			if(GUILayout.Button("+")){
				target.triggers.Add(new AudioPosTriggerData(0, "", ""));
				//Sort triggers
