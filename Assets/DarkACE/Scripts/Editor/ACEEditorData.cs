...
public class ACEEditorData  : EditorWindow
{
	Texture2D musicTex;
	AudioClip clip;
...
//	int selectedHandle = 0;
	...
	private List<AudioPosTriggerDataGUI> triggerPoints;
	...
	public ACEEditorData(){
	}

	[MenuItem("Window/ACE Editor with Data")]
	public static void ShowWindow()
	{
		//Show existing window instance. If one doesn't exist, make one.
		ACEEditorData speedEditor = EditorWindow.GetWindow(typeof(ACEEditorData )) as ACEEditorData;
	...
	void ClearTriggers()
	{
		triggerPoints = new List<AudioPosTriggerDataGUI>();
	}

	void AddTriggerPoint(AudioPosTriggerData point){
		triggerPoints.Add(new AudioPosTriggerDataGUI(point));
	}
	...
	void OnSelectionChange()
	{
		if(Selection.activeGameObject != null){
			AudioEventsData tmpAudioEvents = Selection.activeGameObject.GetComponent<AudioEventsData>();
			if(tmpAudioEvents != null && tmpAudioEvents.GetComponent<AudioSource>().clip != null){
				audioEvents = tmpAudioEvents;
                audioSource = audioEvents.GetComponent<AudioSource>();
...
//		}
	...
	void OnGUI()
	{
		...
		if(audioEvents != null){
			...
			Rect labelsRect = GUILayoutUtility.GetRect(position.width, 15f);
			Rect triggersRect = GUILayoutUtility.GetRect(position.width, 25f);
			...
			//Vertical line
			float pointx = CalculateGUIAudioPosition(zoomFactor, hScrollPosition, guiRect, clip, audioSource.time);

			bool lineUpdating = posLine.OnGUI(new Rect(pointx, 0, 1, guiRect.height));
			if(lineUpdating){
				audioSource.time = CalculateAudioPosition(zoomFactor, hScrollPosition, guiRect, clip, posLine.Position.x);
			}
			...
			float startx = ACEEditorData.CalculateGUIAudioPosition(zoomFactor, hScrollPosition, guiRect, clip, 0);
			float dist = ACEEditorData.CalculateGUIAudioPosition(zoomFactor, hScrollPosition, guiRect, clip, 1) - startx;
			for(int i=0; i<clip.length; i+=5){
				float xpoint = startx + dist * i;
				if(xpoint > leftSidebarWidth && xpoint < position.width){
...
//						oldBezierLHandle = oldBezierRHandle = null;
			...
			if(Event.current.type == EventType.MouseDown && guiRect.Contains(Event.current.mousePosition) && Event.current.shift && audioEvents.curves.Count>0 && selectedCurve < audioEvents.curves.Count){
				...
				float audioPos = ACEEditorData.CalculateAudioPosition(zoomFactor, hScrollPosition, guiRect, clip, Event.current.mousePosition.x);
				float speed = ACEEditorData.CalculateSpeed(guiRect, clip, Event.current.mousePosition.y);
...
//			GUI.DrawTexture(new Rect(0, 0, 24f, Screen.height), bgTex);
			...
			triggersRectMod.width = position.width;
			if(trigBgTex == null){
...
//						Undo.RecordObject(audioCurves, "Remove Trigger");
				...
				if(Event.current.type == EventType.ContextClick && triggerPoints[i].drawRect.Contains(Event.current.mousePosition)){
					...
					dataContent = triggerPoints[i].trigger.data;
					doRepaint = true;
				}
			}
			...
			if(Event.current.type == EventType.MouseDown && triggersRect.Contains(Event.current.mousePosition) && Event.current.shift){
				AudioPosTriggerData newTrigger = new AudioPosTriggerData(ACEEditorData.CalculateAudioPosition(zoomFactor, hScrollPosition, guiRect, clip, Event.current.mousePosition.x), "", "");
//				Undo.RecordObject(audioCurves, "Add Trigger");
			...
			GUILayout.Label(audioSource.time+"", GUILayout.Width(50));
			if(GUILayout.Button("Play", GUILayout.Width(80))){
				audioSource.Play();
			}
			if(GUILayout.Button(audioSource.isPlaying?"Pause":"Stop", GUILayout.Width(80))){
			...
			if(selectedTrigger != -1){
				if(!triggerWasSelected){
					triggerWindowRect = new Rect(triggerPoints[selectedTrigger].drawRect.x - 80, guiRect.height-100, 170, 140);
	...
	void TriggerEditWindow(int windowID) {
		...
		GUI.Label(new Rect(10, 65, 150, 20), "Data:");
		dataContent = GUI.TextField(new Rect(10, 85, 150, 20), dataContent);
		if(GUI.Button (new Rect(5, 110, 75, 20), "Save" )){
			triggerPoints[selectedTrigger].trigger.methodName = sendMessageContent;
			triggerPoints[selectedTrigger].trigger.data = dataContent;
			selectedTrigger = -1;
			triggerWasSelected = false;
			
			Undo.RecordObject(audioEvents, "Changed method name");
		}
		if(GUI.Button (new Rect(85, 110, 75, 20), "Close" )){
			selectedTrigger = -1;
			triggerWasSelected = false;
		}
